"""
End-to-end B1 Hören generation pipeline.

Reusable from BOTH the offline CLI (`agents._tools.horen_*_generator`) AND
the on-demand API endpoint (`POST /horen/{slug}/quiz`). The CLI tools are
thin wrappers around `generate_scripts` and `render_audio_for_script`; the
endpoint calls `generate_session` to produce a complete persisted-ready
...
# Goethe-Zertifikat B1 Hören exam structure).
ITEMS_PER_SESSION = {1: 5, 2: 1, 3: 1, 4: 1}

# Goethe-Zertifikat B1 Hören play rules — most Teile play once, Teil 4 plays twice.
PLAY_LIMIT_PER_TEIL = {1: 1, 2: 1, 3: 1, 4: 2}

INSTRUCTIONS_PER_TEIL = {
    1: "Sie hören fünf kurze Texte. Sie hören jeden Text einmal. Wählen Sie für jede Aufgabe die richtige Antwort.",
    2: "Sie hören einen Vortrag. Sie hören den Text einmal. Wählen Sie für jede Aufgabe die richtige Antwort.",
    3: "Sie hören ein Gespräch. Sie hören das Gespräch einmal. Entscheiden Sie, ob die Aussagen richtig oder falsch sind.",
...
def _teil_chapter_slug(teil: int) -> str:
    return next(
        (c["slug"] for c in DEUTSCH_B1_HOREN_CHAPTERS if c["slug"].startswith(f"teil_{teil}_")),
        f"teil_{teil}",
    )
...
def generate_one_script(teil: int, client=None, *, chosen_topic: Optional[str] = None) -> dict:
    ...
    # google-genai accepts generation params via the `config` kwarg; we pass a
    # plain dict so we don't hard-import the types module just for this.
...
def render_audio_for_script(provider, script: dict, out_path: Path) -> dict:
    ...
    in `agents._tools.horen_audio_generator` so the script + audio logic stays
    in one place."""
    # Imported here to avoid a circular import at module load time and to
    # keep the heavy TTS deps out of pure-script-gen callers.
...
def _flatten_questions(script: dict, teil: int, audio_url: str) -> list[dict]:
    ...
    if teil == 2:
        # 5 MCQs in `questions` array, each with `correct_index`
    ...
    if teil == 3:
        # 7 richtig/falsch items with `answer: 'richtig' | 'falsch'`
        # Normalize to single_choice with options = ['Richtig', 'Falsch']
        for q in script.get("questions") or []:
    ...
    if teil == 4:
        # 8 'who said what' MCQs with options = speaker names
        for q in script.get("questions") or []:
            out.append({
                "topic": topic,
                "stem": q.get("stem", ""),
...
def generate_session(
    ...
    *,
    ...
    """Generate a complete Hören session: scripts + audio + manifest.

    Args:
        teil: which Teil (1–4).
        audio_dir: directory where MP3s are written. Files are named with
                   UUIDs to avoid collisions across user sessions.
        provider_name: TTS provider key from the PROVIDERS dict.
        audio_url_prefix: URL prefix that the StaticFiles mount serves the
                         audio dir under (default matches main.py).
    ...
    Returns:
        A manifest dict ready to drop into Quiz.content. Shape:
          { teil, teil_name, instructions, play_limit,
            audio_segments: [{audio_url, context, duration_seconds}, ...],
            questions:      [...flat list of single_choice questions...] }
    """
    ...
    for i, script in enumerate(scripts):
        ...
        audio_segments.append({
            ...
            "duration_seconds": script.get("estimated_duration_seconds", 30),
        })
...
def session_title(teil: int) -> str:
    """Default title for a generated single-Teil Hören Quiz row (legacy)."""
    return f"German B1 Hören — {teil_name(teil)}"


def full_exam_title() -> str:
    """Default title for a full 4-Teil exam Quiz row."""
    return "German B1 Hören — Vollständige Prüfung"
...
def generate_full_exam(
    ...
    *,
    ...
    """Generate a complete 4-Teil Hören exam matching the real Goethe-Zertifikat
    B1 format. Returns a manifest with each Teil as a section AND a flat
